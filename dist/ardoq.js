...
export class ArdoqError extends Error {
  constructor(message, status, data) {
    ...
    this.status = status;
    this.data = data;
    logger.error(`ArdoqError: ${message}`, {
      status,
      data
    });
  }
...
export class ArdoqClient {
  constructor(token = process.env.ARDOQ_API_TOKEN, options = {}) {
    ...
    const baseURL = options.baseURL || `https://${this.subdomain || "app"}.ardoq.com/api/v2`;
    ...
    this.axiosInstance.interceptors.response.use(response => response, error => Promise.reject(this._handleError(error)));
  ...
  _handleError(error) {
    if (error.response) {
      ...
      const errorMessage = `Error ${method} ${endpoint}: ${error.response.data.message || "API error"}`;
...
class References {
  ...
  async update(id, data, ifVersionMatch) {
    const response = await this.client.axiosInstance.patch(`/references/${id}`, data, {
      params: {
        ifVersionMatch
      }
    });
...
class Components {
  ...
  async update(id, data, ifVersionMatch) {
    const response = await this.client.axiosInstance.patch(`/components/${id}`, data, {
      params: {
        ifVersionMatch
      }
    });
...
class Workspaces {
  ...
  async context(id) {
    const response = await this.client.axiosInstance.get(`/workspaces/${id}/context`);
    return response.data;
  }
...
class Reports {
  ...
  async runObjects(id, params = {}) {
    const response = await this.client.axiosInstance.get(`/reports/${id}/run/objects`, {
      params
    });
    return response.data;
  }
  async runTabular(id) {
    const response = await this.client.axiosInstance.get(`/reports/${id}/run/tabular`);
    return response.data;
  }
}